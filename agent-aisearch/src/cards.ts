...
export function buildSearchResultsCard(
  ...
  const citationBlocks = results.map((r, i) => {
    ...
    const meta: string[] = [];
    if (r.citation.author) meta.push(`👤 ${r.citation.author}`);
    if (r.citation.department) meta.push(`🏢 ${r.citation.department}`);
    if (r.citation.documentVersion) meta.push(`v${r.citation.documentVersion}`);

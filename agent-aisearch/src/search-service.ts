...
export class SearchService {
  ...
  async rewriteQuery(query: string): Promise<string[]> {
    ...
    try {
      ...
      const token = await credential.getToken("https://cognitiveservices.azure.com/.default");
      const resp = await fetch(
        `${this.aoaiEndpoint}/openai/deployments/${this.aoaiChatDeployment}/chat/completions?api-version=2024-06-01`,
        {
          method: "POST",
          headers: { Authorization: `Bearer ${token.token}`, "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async generateAnswer(query: string, context: SearchResultWithCitation[]): Promise<string> {
    const credential = this.getCredential();
    const token = await credential.getToken("https://cognitiveservices.azure.com/.default");
    ...
    const resp = await fetch(
      `${this.aoaiEndpoint}/openai/deployments/${process.env.AZURE_OPENAI_ANSWER_DEPLOYMENT || "gpt-4o"}/chat/completions?api-version=2024-06-01`,
      {
        method: "POST",
        headers: { Authorization: `Bearer ${token.token}`, "Content-Type": "application/json" },
        body: JSON.stringify({

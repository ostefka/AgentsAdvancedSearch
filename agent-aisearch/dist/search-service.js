...
class SearchService {
    ...
    async callOpenAI(deployment, messages, options) {
        const credential = this.getCredential();
        const token = await credential.getToken("https://cognitiveservices.azure.com/.default");
        const resp = await fetch(`${this.aoaiEndpoint}/openai/deployments/${deployment}/chat/completions?api-version=2024-06-01`, {
            method: "POST",
            headers: { Authorization: `Bearer ${token.token}`, "Content-Type": "application/json" },
            body: JSON.stringify({
                messages,
     ...
     */
    async rewriteQuery(query, conversationHistory) {
...
Return ONLY a JSON array of strings. Example: ["query1", "query2", "query3"]`,
     ...
     */
    async smartSearch(query, options, userToken, conversationHistory) {
        ...
        const searchPromises = queries.map(q => this.search(q, { top: top + 3, filter, searchType: "hybrid" }, userToken)
            .catch(err => {
        ...
        if (avgScore < 0.3 && ranked.length < top) {
            ...
            try {
                ...
                if (Array.isArray(refinedQueries)) {
                    const refinedPromises = refinedQueries.slice(0, 2).map(q => this.search(q, { top, filter, searchType: "hybrid" }, userToken).catch(() => []));
    ...
    async generateAnswer(query, context, conversationHistory) {
        const contextText = context.map((r, i) => {
            ...
            if (r.citation.author)
                meta.push(`Author: ${r.citation.author}`);
            if (r.citation.department)
                meta.push(`Department: ${r.citation.department}`);
            if (r.citation.documentVersion)
                meta.push(`Version: ${r.citation.documentVersion}`);
            if (r.citation.lastModified)
                meta.push(`Modified: ${r.citation.lastModified}`);
